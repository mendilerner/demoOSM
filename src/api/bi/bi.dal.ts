...
const getOrdersByDateExecution = async (): Promise<CompletedOrdersInterface[]> => {
  try {
    const result = await Shipping.aggregate([
      {
        $group: {
          _id: { $dateToString: { format: '%d/%m/%Y', date: '$orderDateExecution' } },
        ...
        $addFields: {
          ...
          month: {
            $let: {
              ...
              in: {
                $arrayElemAt: ['$$monthsArray', { $subtract: [{ $month: '$orderTime' }, 1] }]
              }
            }
          }
        }
    ...
    const topProducts = await OrderModel.aggregate([
      ...
      },
      {
        $group: {
          ...
          profits: { $sum: { $multiply: ['$cartItems.profit', '$cartItems.quantity'] } },
...
export default {getShipping, getOrdersByDateExecution, getProfitsPerMonth, getTopProfitableProducts}